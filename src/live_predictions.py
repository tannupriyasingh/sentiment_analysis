...
if __name__ == '__main__':
    ...
    live_prediction.make_predictions()
    live_prediction = LivePredictions(file=EXAMPLES_PATH + 'su15.wav')
    live_prediction.make_predictions()
    live_prediction = LivePredictions(file=EXAMPLES_PATH + 'h11.wav')
    live_prediction.make_predictions()


    from pathlib import Path

    all_wav_dir = Path('examples/tannu')
    all_wavs = [f for f in all_wav_dir.rglob("*.wav")]

    res = []
    for f in all_wavs[:100]:
        live_prediction = LivePredictions(file=str(f))
